...
//input1 is max number of attacks each can choose.
//input2 is strength of each attack
//input3 is required strength of the combo
//output is if a valid combo of 4 attacks can produce input3
int validCombo(int input1,const int attack[],int input3)
{
	//Write code here
	int totalAttack=0;
	for(int i=1;i<=input1;i++)
	{
		cout<<i<<"th attack is"<< attack[i]<<endl;
	}
	...
	vector<int> mulattack;
	int distinctattack=0;
	for(int i=1;i<=input1;i++)
		for(int j=1;j<=input1;j++)
			for(int k=1;k<=input1;k++)
				for(int l=1;l<=input1;l++)
				{
					totAttack=attack[i]+attack[j]+attack[k]+attack[l];
					if(totAttack==input3)
					{
						if(distinctattack==0)	//add first possible attack
						{
							distinctattack++;
							mulattack.push_back(attack[i]*attack[j]*attack[k]*attack[l]);
							cout<<"combo attack possible is "<<attack[i]
							<<","<<attack[j]<<","<<attack[k]<<","<<attack[l]<<endl;
						}
						else
						{
							int m;
							for(m=0;m<distinctattack;m++)
								if(mulattack[m]==attack[i]*attack[j]*attack[k]*attack[l])
									break;
