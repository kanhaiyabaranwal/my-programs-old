...
int find_line_covering_maximum_points(const vector<point>& ivec)
{
	...
	unordered_map<Line,vector<point>, hash_line > hmap;
	for(int i=0;i<ivec.size();i++)
	{
		for(int j=i;j<ivec.size();j++)
		{
			...
			hmap[l].push_back(ivec[i]);
			hmap[l].push_back(ivec[j]);
		}
	}
	unordered_map<Line,vector<point>, hash_line >::const_iterator iter=hmap.begin();
	...
	for(int l=0;iter!=hmap.end();iter++,l++)
	{
		...
		vector<point>::const_iterator siter=iter->second.begin();
		cout<<"point details: ";
		int i;
		for(i=0;siter!=iter->second.end();siter++,i++)
				cout<<"point["<<i+1<<"]="<<(*siter).x<<","<<(*siter).y<<endl;
		if(i>maxpoint)
			maxpoint=i;
	}
	cout<<"maximum number of points through a line is "<<maxpoint<<endl;
	maxpoint=0;
	for(const pair<Line,vector<point> >& p:hmap)
