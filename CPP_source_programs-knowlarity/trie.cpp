...
void insert(trie *pTrie, char key[],int id,bool type)	//type is for exact or prefix match;type=0-exact,1-prefix
{
	...
	//handle case
	/*if(key[level]>64 && key[level] < 91)
	{
		if(pCrawl->children[index]->case==2)
			pCrawl->children[index]->case=3;
		else
			pCrawl->children[index]->case=1;
	}
	else if(key[level]>96 && key[level] < 123)
	{
		if(pCrawl->children[index]->case==1)
			pCrawl->children[index]->case=3;
		else
			pCrawl->children[index]->case=2;
	}*/

	//handle exact or prefix match
	...
	//cout<<"pCrawl->exact_match_id="<<pCrawl->exact_match_id<<endl;
	//cout<<"pCrawl->prefix_match_id="<<pCrawl->prefix_match_id<<endl;
	//cout<<"id="<<id<<endl;
}
...
int search(trie *pTrie, char key[],bool type)
{
    ...
    for( level = 0; level < length; level++ )
    {
	cout<<"Navigating char->"<<key[level]<<endl;
        index = CHAR_TO_INDEX(key[level]);
	...
	{
		cout<<"Prefix match id="<<pCrawl->children[index]->prefix_match_id<<endl;
		id=pCrawl->children[index]->prefix_match_id;
	}
	cout<<"type="<<type<<" and id="<<id<<endl;
