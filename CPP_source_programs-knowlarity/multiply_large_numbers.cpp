//Multiply two large integers using Linked List. Number of digits in each number may overflow integer size.
...
void multiply_ll(node *num1,node *num2,node *mid_node,int *gap,node **sum_node)
{
    node *temp;
    int carry=0;
    if(num2->next==NULL)
    ...
    {
        multiply_ll(num1,num2->next,mid_node,gap,sum_node);
        multiply_node(num1,num2,mid_node->next,&carry);
        if(carry)
            mid_node->data=carry;
        else
...
int main()
{
    ...
    int gap=0;
    cout<<"\nNumbers from LinkedList"<<endl;
    cout<<"Number 1:";
    show_ll(num1);
    cout<<"Number 2:";
    show_ll(num2);

    multiply_ll(num1,num2,mid_node,&gap,&sum_node);
